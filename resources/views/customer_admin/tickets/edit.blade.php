  ...
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-4">
    <div class="flex items-start justify-between">
      ...
      <div class="flex items-center gap-2">
        <form action="{{ route('customer-admin.tickets.destroy', $ticket) }}" method="POST" onsubmit="return confirm('Delete this ticket? This action cannot be undone.')">
          @csrf
          @method('DELETE')
          <button class="px-3 py-1 rounded bg-red-600 text-white text-sm hover:bg-red-700">Delete</button>
        </form>
      </div>
    </div>
    ...
    <form action="{{ route('customer-admin.tickets.update', $ticket) }}" method="POST">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <select name="contact_id" class="w-full rounded-lg border px-3 py-2 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
            <option value="">— Select contact —</option>
            @foreach($contacts as $c)
              <option value="{{ $c->id }}" {{ old('contact_id', $ticket->contact_id) == $c->id ? 'selected' : '' }}>
        ...
        <div>
          ...
          <select name="assignee_id" class="w-full rounded-lg border px-3 py-2 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
            <option value="">Unassigned</option>
            @php $assignees = $agents ?? ($users ?? collect()); @endphp
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <div class="flex items-center gap-2">
            <label class="inline-flex items-center gap-2 text-sm">
              <input type="radio" name="priority" value="normal" {{ old('priority', $ticket->priority) === 'normal' ? 'checked' : '' }} />
        ...
        <div>
          ...
          <select name="status" class="w-full rounded-lg border px-3 py-2 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
      ...
      <div class="mb-4">
        ...
        <textarea id="ticketBody" name="body" rows="8" class="w-full rounded-lg border px-3 py-2 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 resize-y">{{ old('body', $ticket->body) }}</textarea>
      </div>

      {{-- Actions --}}
      <div class="flex items-center gap-3">
        <button type="submit" class="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Save changes</button>
        <a href="{{ route('customer-admin.tickets.index') }}" class="px-4 py-2 rounded bg-gray-100 dark:bg-gray-700 text-sm">Cancel</a>
      </div>
    </form>
  </div>
</div>
